...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🎾 Bot attivo! Usa /help per istruzioni.")

async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(HELP_TEXT)

async def retrain_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = await update.message.reply_text("⏳ Avvio re-train del modello...")
...
async def callback_query_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if typ == "DETAILS":
        ...
        reasons=[]
        if feat['hand_adj'] < 0:
            reasons.append("Destro vs Mancino → svantaggio per il destro")
        if feat['form_p1'] > feat['form_p2'] + 0.3:
            reasons.append(f"{p1} è in forma recente")
        if feat['h2h_total']>0:
